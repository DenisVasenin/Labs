...
// ===================================== ОБЬЯВЛЕНИЕ СТРУКТУРЫ И КЛАССОВ ======================================
...
class List {
	...
	public:
		...
		void insertElement(Element* Start, int Num, int Val);
		void deleteElement(Element* Start, int Num);
...
class Stack:private List {
	public:
		Element* createStack(void);
		void deleteStack(Element* Head);
		void printStack(Element* Head);
		int lengthStack(Element* Head);
		void push(Element* Head, Element* Addr);
		void push(Element* Head, int Val);
...
/*
	...
	Возвращает указатель на элемент если он найден. В противном случае возвращает NULL.
	Пример:
...
void List::addElement(Element* Head, int Val) {
	...
	AddrNew->data = Val;
	AddrNew->next = NULL;
	AddrPrev = getElementAddress(Head, lengthList(Head));
...
/*
	...
	Пример:
		...
		insertElement(Start, 4, 56);                      // добавляем новый элемент списка в позицию 3+4 c начала списка (элементы с 1 по 6 уже должны существовать)
		
		insertElement(Head, 2, 56);                       // добавляем новый элемент списка в позицию 2 c начала списка (элемент 1 уже должен существовать)
*/

void List::insertElement(Element* Start, int Num, int Val) {
	Element* AddrNew;
	Element* AddrPrev;
	AddrNew = createElement();
...
/*
	...
	Пример:
		...
		deleteElement(Start, 2);                          // удаляем элемент списка в позиции 8+2 c начала списка (элемент 10 уже должен существовать)
		
		deleteElement(Head, 5);                           // удаляем элемент списка в позиции 5 c начала списка (элемент 5 уже должен существовать)
*/

void List::deleteElement(Element* Start, int Num) {
	Element* AddrDel;
	Element* AddrPrev;
	AddrPrev = getElementAddress(Start, Num-1);
...
void List::printListData(Element* Head) {
	...
	int Count;
	cout << Head->next << " => ";
	for (Count = 1; Count <= lengthList(Head); Count++) {
...
void List::deleteList(Element* Head) {
	while (Head->data) deleteElement(Head, 1);
	free(Head);
}
...
Element* Stack::createStack(void) {
	return(createList());
}


void Stack::deleteStack(Element* Head) {
	deleteList(Head);
}


void Stack::printStack(Element* Head) {
	printListData(Head);
}


int Stack::lengthStack(Element* Head) {
	return(lengthList(Head));
}


void Stack::push(Element* Head, Element* Addr) {
	Addr->next = Head->next;
	Head->next = Addr;
}


void Stack::push(Element* Head, int Val) {
	insertElement(Head, 1, Val);
}


Element* Stack::pop(Element* Head) {
	Element* Addr; 
	Addr = getElementAddress(Head, 1);
	Head->next = getElementAddress(Head, 2);
	return(Addr);
}
